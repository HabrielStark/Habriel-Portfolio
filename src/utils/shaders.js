...
export const sunFragmentShader = `
  ...
  void main() {
    float n = noise(vec3(vUv * 5.0 + time * 0.2, time * 0.1));
    float intensity = 1.0 - dot(vNormal, vec3(0.0, 0.0, 1.0));
    vec3 finalColor = mix(color, color * 1.5, n);
...
export function createSunTexture() {
  ...
  // Создаем градиент для солнечной поверхности
  const gradient = ctx.createRadialGradient(256, 256, 0, 256, 256, 256);
  gradient.addColorStop(0, '#FDB813');
  gradient.addColorStop(0.5, '#FCA82F');
